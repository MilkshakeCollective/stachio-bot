...
// how many users to pull from DB per batch
const BATCH_SIZE = 100;

/**
 * Poll DB for blacklisted guilds not yet notified and send them to the channel
 * Processes guilds in batches so Prisma never returns too much data at once.
 */
 ...
 * Processes users in batches so Prisma never returns too much data at once.
 */
export async function pollNewBlockedUsers(client: HelperClient) {
	...
	let hasMore = true;
	let lastId = 0; // use id cursor to paginate
